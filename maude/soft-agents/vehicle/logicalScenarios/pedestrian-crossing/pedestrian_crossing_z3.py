...
term =  m.parseTerm(asys)
ASystem = m.parseTerm("d:ASystem")
...
queue.append((term, 0, "init"))
while queue and n < max:
    current, depth, label = queue.popleft()

    stop = m.parseTerm(f"getStopTime({current})")
    stop.reduce()
    print(stop)

    t = m.parseTerm(f"getTime({current})")
    t.reduce()

    ekb = m.parseTerm(f"envKB({current})")
    ekb.reduce()

    nzt = m.parseTerm(f"mte(getConf({current}))")
    nzt.reduce()
    n = n+1
    print(current.getSort())
    print("count:", n)
    print("depth", depth)
    print("time", t)
    print("rule", label)
    ...
    for sol, rew, pathTo, nrew in term.search(maude.ONE_STEP, ASystem):
        queue.append((sol, depth+1, pathTo()[1].getLabel()))
